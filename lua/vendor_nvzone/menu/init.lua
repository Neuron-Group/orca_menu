local M = {}
local api = vim.api
local state = require "vendor_nvzone.menu.state"
local layout = require "vendor_nvzone.menu.layout"
local ns = api.nvim_create_namespace "NvMenu"
local volt = require "vendor_nvzone.volt"
local volt_events = require "vendor_nvzone.volt.events"
local mappings = require "vendor_nvzone.menu.mappings"
...
M.open = function(items, opts)
  ...
  bufv.w = require("vendor_nvzone.menu.utils").get_width(items)
  bufv.w = bufv.w + bufv.item_gap

  local win_opts = {
    relative = config.relative or (config.mouse and "mouse" or "cursor"),
    width = bufv.w,
    height = h,
    row = config.row or 1,
    col = config.col or 0,
