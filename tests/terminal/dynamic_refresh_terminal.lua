...
timer:start(80, 80, vim.schedule_wrap(function()
  ...
  elseif phase == "update_menu" then
    if not popup.is_open() then
      local elapsed_ms = (vim.loop.hrtime() - start) / 1000000
      if elapsed_ms >= timeout_ms then
        timer:stop()
        timer:close()
        fail("timed out waiting for runtime menu popup to open")
      end
      return
    end
    ...
    orca.update_menu("tools", {
      ...
      key = "t",
      items = {
        { label = "Run &Two", key = "2", action = function() end },
      },
    })
    phase = "verify_update"
    return
  elseif phase == "verify_update" then
    local first_item = state.menu_stack[1] and state.menu_stack[1].items and state.menu_stack[1].items[1]
    if not popup.is_open() then
      timer:stop()
      timer:close()
      fail("expected popup to remain open across update_menu refresh")
      return
    end
    if state.active_top ~= 2 then
      timer:stop()
      timer:close()
      fail("expected active_top to stay on runtime menu across update_menu refresh")
      return
    end
    if not first_item or first_item.label ~= "Run Two" then
      timer:stop()
      timer:close()
      fail("expected updated runtime popup contents after update_menu refresh")
      return
    end

    orca.refresh()
    phase = "verify_refresh"
    return
  elseif phase == "verify_refresh" then
    local first_item = state.menu_stack[1] and state.menu_stack[1].items and state.menu_stack[1].items[1]
    if not popup.is_open() then
      timer:stop()
      timer:close()
      fail("expected popup to remain open across explicit refresh")
      return
    end
    if state.active_top ~= 2 then
      timer:stop()
      timer:close()
      fail("expected active_top to stay on runtime menu across explicit refresh")
      return
    end
    if not first_item or first_item.label ~= "Run Two" then
      timer:stop()
      timer:close()
      fail("expected runtime popup contents to survive explicit refresh")
      return
    end
  ...
  if elapsed_ms >= timeout_ms then
    timer:stop()
    timer:close()
    fail("timed out waiting for dynamic refresh scenario")
  end
end))
