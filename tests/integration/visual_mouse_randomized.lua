...
local lines = vim.fn.readfile(vim.fn.getcwd() .. "/tests/samples/sample_01")
H.truthy(#lines > 12, "sample fixture should have enough lines for randomized selection coverage")
...
for attempt = 1, 60 do
	...
	if not expected.selection then
		fail(
			...
			string.format(
				"attempt=%d seed=%d start=%d:%d end=%d:%d",
				attempt,
				seed,
				start_row,
				start_col,
	...
	H.truthy(
		state.menu_context and state.menu_context.selection,
		"mouse click should preserve visual selection context"
	)
	H.eq(state.menu_context.selection.mode, expected.selection.mode, "mouse click should preserve selection mode")
	H.eq(
		state.menu_context.selection.start_row,
		expected.selection.start_row,
		"mouse click should preserve selection start row"
	)
	H.eq(
		state.menu_context.selection.start_col,
		expected.selection.start_col,
		"mouse click should preserve selection start col"
	)
	H.eq(
		state.menu_context.selection.end_row,
		expected.selection.end_row,
		"mouse click should preserve selection end row"
	)
	H.eq(
		state.menu_context.selection.end_col,
		expected.selection.end_col,
		"mouse click should preserve selection end col"
	)
